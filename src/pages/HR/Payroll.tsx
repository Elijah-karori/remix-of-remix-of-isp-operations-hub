...
export default function Payroll() {
    ...
    // Calculate total from gross_amount or net_amount
    const totalPending = payouts?.reduce((sum, p) => {
        const amount = typeof p.gross_amount === 'string' ? parseFloat(p.gross_amount) : (p.gross_amount || 0);
        return sum + amount;
    }, 0) || 0;

    return (
        <DashboardLayout title="Payroll" subtitle="Review and approve employee earnings">
                    ...
                    <CardContent className="p-0">
                        {isLoading ? (
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {payouts?.map((payout) => {
                                        const grossAmount = typeof payout.gross_amount === 'string' ? parseFloat(payout.gross_amount) : (payout.gross_amount || 0);
                                        const netAmount = typeof payout.net_amount === 'string' ? parseFloat(payout.net_amount) : (payout.net_amount || 0);
                                        
                                        return (
                                            <TableRow key={payout.id}>
                                                <TableCell className="pl-6 font-medium">#{payout.employee_id}</TableCell>
                                                <TableCell className="text-xs">
                                                    {new Date(payout.period_start).toLocaleDateString()} - {new Date(payout.period_end).toLocaleDateString()}
                                                </TableCell>
                                                <TableCell>Ksh {grossAmount.toLocaleString()}</TableCell>
                                                <TableCell className="font-bold text-foreground">Ksh {netAmount.toLocaleString()}</TableCell>
